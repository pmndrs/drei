//* Legacy Materials ==============================

export * from './AccumulativeShadows'
export * from './BakeShadows'
export * from './BlurPass'
export * from './Caustics'
export * from './ContactShadows'
export * from './ConvolutionMaterial'
export * from './DiscardMaterial'
export * from '../Textures/GradientTexture'
export * from './Image'
export * from './MeshDiscardMaterial'
export { MeshDistortMaterial } from './MeshDistortMaterial'
export type { MeshDistortMaterialProps } from './MeshDistortMaterial'
export * from './MeshPortalMaterial'
export * from './MeshReflectorMaterial'
export * from './MeshRefractionMaterial'
export * from './MeshTransmissionMaterial'
export { MeshWobbleMaterial } from './MeshWobbleMaterial'
export type { WobbleMaterialProps } from './MeshWobbleMaterial'
export * from '../../experimental/Effects/Outlines'
export * from './shaderMaterial'
export * from './SoftShadows'
export * from './SpotLightMaterial'
export * from './WireframeMaterial'
export * from './CloudMaterial'
export * from './StarsMaterial'
export * from './SparklesMaterial'