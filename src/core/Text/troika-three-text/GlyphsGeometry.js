  ...
  if (!geom) {
    ...
    ;['position', 'normal', 'uv'].forEach((name) => {
      combined.setAttribute(
        name,
        new Float32BufferAttribute([...frontAttrs[name].array, ...backAttrs[name].array], frontAttrs[name].itemSize)
      )
    })
...
class GlyphsGeometry extends InstancedBufferGeometry {
  ...
  set detail(detail) {
    if (detail !== this._detail) {
      ...
      const tpl = getTemplateGeometry(detail)
  ...
  applyClipRect(clipRect) {
    ...
    const chunks = this._chunkedBounds
    if (chunks) {
      for (let i = chunks.length; i--; ) {
        count = chunks[i].end
        const rect = chunks[i].rect
