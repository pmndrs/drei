...
import { CameraShake, OrbitControls } from 'drei'
import { ComponentProps } from 'react'

export default {
  title: 'Effects/CameraShake',
  component: CameraShake,
  decorators: [
    (Story, context) => (
      <Setup renderer={context.globals.renderer} cameraPosition={new THREE.Vector3(0, 0, 10)} controls={false}>
        <Story />
      </Setup>
    ),
  ],
...
function CameraShakeScene1(props: ComponentProps<typeof CameraShake>) {
  ...
  useFrame((state) => {
    if (cube.current) {
      // Use elapsed time for deterministic rotation
      cube.current.rotation.x = cube.current.rotation.y = state.elapsed * 0.3
    }
  })
