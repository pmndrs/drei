...
(cd $appdir; npm i three@${THREE_VERSION_FULL} @react-three/fiber@^9.0.0)
...
# create app (pipe "no" to skip React Compiler prompt)
(cd $tmp; echo "n" | npx -y create-next-app@latest $appname --ts --no-eslint --no-tailwind --no-src-dir --app --import-alias "@/*" || true)
