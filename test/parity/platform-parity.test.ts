...
const FALLBACK_STORIES = ['helpers-clone--clone-st', 'controls-dragcontrols--drag-controls-story', 'shapes-box--box-st']
  ...
  for (const storyId of parityStories) {
    ...
    try {
      ...
      if (legacyPng.width !== webgpuPng.width || legacyPng.height !== webgpuPng.height) {
        console.log(
          `   ❌ Dimension mismatch: Legacy ${legacyPng.width}x${legacyPng.height} vs WebGPU ${webgpuPng.width}x${webgpuPng.height}`
        )
      ...
      const mismatchedPixels = pixelmatch(legacyPng.data, webgpuPng.data, diff.data, width, height, {
