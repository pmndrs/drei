...
type FaceControlsProps = {
  ...
  /** Callback function to call on "EOF_MARKERrame" event, default: undefined */
  EOF_MARKERrame?: (e: THREE.Event) => void
...
- `{ type: "EOF_MARKERrame", texture: THREE.VideoTexture, time: number }` -- each time a new video frame is sent to the compositor (thanks to rVFC)

> [!Note]
> rVFC
>
> Internally, `FaceControls` uses [`EOF_MARKERrameCallback`](https://caniuse.com/mdn-EOF_MARKERramecallback), you may need [a polyfill](https://github.com/ThaUnknown/rvfc-polyfill) (for Firefox).

## FaceControls[manualDetect]

By default, `detect` is called on each `"EOF_MARKERrame"`. You can disable this by `manualDetect` and call `detect` yourself.
